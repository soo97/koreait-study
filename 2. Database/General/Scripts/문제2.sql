...
SELECT * FROM employees ORDER BY gender ASC, emp_no ASC;
...
SELECT gender, count(*) FROM employees GROUP BY gender HAVING count(*) >= 100000;
-- Q17. 부서별 사원 수 중 20000명 이상인 부서만 조회하세요.
SELECT emp_no, count(*) FROM titles GROUP BY emp_no HAVING count(*) >= 20000;
-- Q18. 직급별 사원 수 중 50000명 이상인 직급만 조회하세요.
SELECT title, count(*) FROM titles GROUP BY title HAVING count(*) >= 50000;
-- Q19. 사번별 평균 급여가 80000 이상인 사원만 조회하세요.
SELECT emp_no, avg(salary) FROM salaries GROUP BY emp_no HAVING avg(salary) >=80000;
